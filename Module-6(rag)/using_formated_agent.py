...
def generate_answer(query, relevant_chunks):
    ...
    payload = {
        "model": GROQ_MODEL,
        "messages": [
            {"role": "system", "content": "You are a helpful assistant. Provide a detailed answer."},
...
# Agent to format the answer into points
def format_answer_as_points(answer):
    points = answer.split(". ")  # Split by sentences for simplicity
...
def handle_query():
    ...
    if relevant_chunks:
        ...
        detailed_answer = generate_answer(user_query, relevant_chunks)
        formatted_answer = format_answer_as_points(detailed_answer)
